...
def get_episodic_semi_gradient_sarsa(env: DeepSingleAgentEnv, gamma: float, epsilon: float,
    ...
    for episode_id in tqdm.tqdm(range(max_iters)):
        ...
        while not env.is_game_over():
            ...
            if env.is_game_over():
                target = r
                q_inputs = np.hstack([s, tf.keras.utils.to_categorical(chosen_action, env.max_actions_count())])
            ...
            q_inputs = np.hstack([s_p, tf.keras.utils.to_categorical(chosen_action, env.max_actions_count())])
